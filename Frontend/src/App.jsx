...
const AppLayout = ({ 
  ...
  const { messages, sendMessage, fetchReports, fetchStaff, fetchCitizens, fetchMessages } = useStore();
  ...
  useEffect(() => {
    ...
    fetchCitizens();
    fetchMessages();
  }, [fetchReports, fetchStaff, fetchCitizens, fetchMessages]);
          ...
          {isChatOpen ? (
            <div className="bg-white w-[calc(100vw-2rem)] h-[70vh] sm:h-[600px] md:w-[400px] md:h-[600px] rounded-[2rem] md:rounded-[3rem] shadow-2xl border border-slate-100 flex flex-col overflow-hidden animate-modal-up mb-4">
              ...
              <div className="flex-1 overflow-y-auto p-5 md:p-8 space-y-4 md:space-y-6 bg-slate-50/50">
                {messages.map((msg) => (
                  <div key={msg._id || msg.id} className={`flex ${msg.isAdmin ? 'justify-start' : 'justify-end'}`}>
